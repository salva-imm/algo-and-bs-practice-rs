    ...
    if list1.is_some() && list2.is_some() {
        ...
        }
        {
            ...
            while list1.is_some() && list2.is_some() {
                if list1.as_ref().unwrap().val < list2.as_ref().unwrap().val {
                    temp_head.as_mut().unwrap().next = Some(Box::new(ListNode::new(list1.as_mut().unwrap().val, None)));
                    temp_head = temp_head.unwrap().next.as_mut();
                    list1 = list1.unwrap().next.take();
                } else {
                    temp_head.as_mut().unwrap().next = Some(Box::new(ListNode::new(list2.as_mut().unwrap().val, None)));
                    temp_head = temp_head.unwrap().next.as_mut();
                    list2 = list2.unwrap().next.take();
                }
...
fn main() {
    let head_1 = ListNode::new(1, Some(Box::new(ListNode::new(3, Some(Box::new(ListNode::new(4, None)))))));
    let head_2 = ListNode::new(1, Some(Box::new(ListNode::new(2, Some(Box::new(ListNode::new(4, None)))))));
    ...
    println!("{:?}", merge_two_lists(Some(Box::new(head_1)), Some(Box::new(head_2))));
}
